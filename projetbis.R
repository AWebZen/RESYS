...
main = function(file, tr, extension)
{
  ...
  write(paste("#Weighted : ",is_weighted, "\tOriented : ",  identical(FALSE,symetric)), file ="output.txt")
  ...
  if (! is_weighted)
  {
    ...
    write.csv(matr, file = "output_BFS.csv", quote = FALSE)
    ...
    colnames(matr) = paste("Node", 1:dim(gr_Adj)[1])
    rownames(matr) = paste("Node", 1:dim(gr_Adj)[1])
    write.csv(matr, file = "output_AM.csv", quote = FALSE)
  ...
  colnames(matr) = paste("Node", 1:dim(gr_Adj)[1])
  rownames(matr) = paste("Node", 1:dim(gr_Adj)[1])
  write.csv(matr, file = "output_FW.csv", quote = FALSE)
